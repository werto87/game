...
int main(int argc, char **argv) {
  ...
  auto const winningPosition = std::pair{static_cast<unsigned short int>(winningPositionX), static_cast<unsigned short int>(winningPositionY)};
  auto const startPoint = std::pair{static_cast<unsigned short int>(startPointX), static_cast<unsigned short int>(startPointY)};
  auto points =std::vector<game::PointWithHistory<unsigned short int>>(static_cast<unsigned>(pointCount), {startPoint, static_cast<unsigned short>(maxExecutions)});
  ...
  while (
    ...
    executionDurations.reserve(static_cast<size_t>(maxExecutions));
...
#endif
      int i=0;
      std::for_each(std::execution::par_unseq, points.begin(), points.end(), [limit, winningPosition, fittest,i](game::PointWithHistory<unsigned short int> &point)mutable {
    ...
    points = std::vector<game::PointWithHistory<unsigned short int>>(static_cast<unsigned>(pointCount), {startPoint, static_cast<unsigned short>(maxExecutions)});
